    ...
    <div className="p-8 max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-gray-800 rounded-2xl border border-gray-700 p-6">
          <h2 className="font-bold text-white mb-4 flex items-center gap-2"><Upload size={16} className="text-amber-500" /> Images</h2>
          <div className="flex flex-wrap gap-3 mb-3">
            ...
            <label className="w-20 h-20 border-2 border-dashed border-gray-600 hover:border-amber-500 rounded-xl flex flex-col items-center justify-center cursor-pointer">
        ...
        <div className="bg-gray-800 rounded-2xl border border-gray-700 p-6">
          <h2 className="font-bold text-white mb-4">Sizes</h2>
          <div className="flex flex-wrap gap-2">
            {ALL_SIZES.map((s) => (
              <button key={s} type="button" onClick={() => toggleSize(s)} className={`px-3 py-2 rounded-xl text-sm font-semibold border transition-all ${sizes.includes(s) ? 'border-amber-500 bg-amber-500/20 text-amber-400' : 'border-gray-600 text-gray-400 hover:border-gray-500'}`}>{s}</button>
            ))}
          </div>
        </div>

        <motion.button type="submit" disabled={loading} whileTap={{ scale: 0.97 }} className="w-full py-4 bg-gradient-to-r from-amber-500 to-orange-500 text-white font-black rounded-xl shadow-lg disabled:opacity-60">
